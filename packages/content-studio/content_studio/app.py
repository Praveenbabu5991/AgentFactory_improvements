...
from content_studio.agent import create_content_studio_agent
...
def _get_agent():
    global _agent
    if _agent is None:
        _agent = create_content_studio_agent(checkpointer=_checkpointer)
    return _agent
...
# Create FastAPI app
# ---------------------------------------------------------------------------
FRONTEND_DIR = Path(__file__).resolve().parent.parent.parent.parent / "frontend" / "content-studio"
...
async def chat_stream(request: ChatRequest):
    ...
    if request.attachments:
        for att in request.attachments:
            ...
            if att_type == "brand_config":
                # Direct brand config (from API/curl tests)
    ...
    # Extract brand name/industry/tone from message text if not in brand dict
...
# ---------------------------------------------------------------------------
# Startup event - register asset routes once agent is available
# ---------------------------------------------------------------------------
...
def main():
    ...
    port = int(__import__("os").getenv("CONTENT_STUDIO_PORT", "5000"))
    print(f"\n🎨 Content Studio Agent starting...")
    print(f"📍 UI: http://localhost:{port}")
    print(f"📍 API Docs: http://localhost:{port}/docs\n")
    uvicorn.run("content_studio.app:app", host=HOST, port=port, reload=DEBUG)
